...
'''class ConvertBags:
	bag_file_name=[] # bag file for conversion
	bag=[]
...
'''
		...
		# 	#ecm_fk_inv = self.ecm_kin.forward([0,0,0,0])**-1
		...
		# 		#ecm_fk_inv = new_pose**-1    # add if you want in ecm cs.
        #
		# 			new_msg = PoseStamped()
		...
		# 			new_pose = ecm_inv * self.psm1_kin.forward(msg.position[0:-1]) #* ecm_fk_inv #add if you want in ecm cs
        #
		# 			new_msg = PoseStamped()
		# 			quaterion_pose = PoseConv.to_pos_quat(new_pose)
		# 			new_msg.pose.position.x = quaterion_pose[0][0]
		...
		# 			ecm_inv =  self.ecm_kin.forward(ecm_last_angles)**-1
		# 			new_pose = ecm_inv * self.psm2_kin.forward(msg.position[0:-1]) # * ecm_fk_inv # add if yu want in ecm cs
        #
		# 			new_msg = PoseStamped()
		# 			quaterion_pose = PoseConv.to_pos_quat(new_pose)
		# 			new_msg.pose.position.x = quaterion_pose[0][0]
...
#usage:	
a = ConvertBags('AutocameraS4P2.bag')
a.ComputeCartesian('AutocameraS4P2.bag')
a.WriteTensorFileCartesian('AutocameraS4P2.csv')
